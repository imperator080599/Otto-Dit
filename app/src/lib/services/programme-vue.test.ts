...
describe('le programme de travail', () => {
  ...
  it("Lot 3, tranches 1-4 — LOT COMPLET — atelierDeLaNature : sondage_pieces/REVENUE, recalcul_parametre/REVENUE, confirmation_externe/CASH et rapprochement/CASH ont un atelier réel ; ailleurs, aucun (règle 13 — jamais un lien menteur)", () => {
    ...
    expect(atelierDeLaNature('rapprochement', 'CASH', '/eng/x')).toBe('/eng/x/circularisations');
    /* Aucun des ateliers ne se prête à un AUTRE poste : `testing` reste câblé
       sur REV-SUBST/REVENUE (Lot 2), `estimations` rapproche sur les comptes
       70x (`montantComptabilise`), `circularisations` sur
    ...
    /* Lot 3 (Partie C.1) est COMPLET avec ces quatre cases : les quatre
       ...
       atelier construit ici — même sur un poste déjà câblé. */
    expect(atelierDeLaNature('test_exhaustif', 'CASH', '/eng/x')).toBeNull();
