...
describe('le rail du dossier (ADR-103, ADR-112)', () => {
  ...
  it('aucun écran de dossier n\'est injoignable : rail, poste, ou déclaré', async () => {
    ...
    const AILLEURS: Record<string, string> = {
      ...
      '/eng/[id]/poste/[code]/detail-compte': 'depuis le bloc « échantillon » de /poste/[code] quand programme.ts::atelierDeLaNature route la nature `rapprochement` (PAYROLL R98, EQUITY R100) ou `observation_documentee` (INVENTORY, R99) du poste — conditionnel à la procédure planifiée, jamais un rail générique',
    };
