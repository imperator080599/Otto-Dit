...
export const SECTIONS: SectionSemeur[] = [
  {
    ...
    objets: [
      ...
      { objet: 'independence_declaration (posée directement, signée)', semeur: 'seed.ts:187-192',
        ...
        clique: 'scenario.ts:515-591 (Lot 4, tranche 3, station « équipe et indépendance »)', etat: 'prouve',
        note: 'les quatre gestes réels sont cliqués dans l’ordre : révision de ma déclaration (motif écrit), chaque rubrique répondue une par une, signature (offerte seulement une fois tout répondu — vérifié), affectation d’un membre (idempotente, sur la personne dont l’indépendance vient d’être reconfirmée par ce même parcours).' },
      { objet: 'client_contact', semeur: 'seed.ts:229-236', cheminHumain: null, clique: null, etat: 'decor',
        note: 'recherche exhaustive : SEUL seed.ts:229 insère cette table dans tout app/src ; ailleurs elle n’est que lue ou jointe (reunions.ts). Citation de ligne corrigée après P2-03b (revue hostile V2-05) : le troisième contact fixture (Zoé, jeton toujours expiré, PORTAIL-02) a déplacé l’INSERT, la classification decor reste inchangée.' },
  ...
  },
  {
    ...
    objets: [
      { objet: 'import TB (dossier SOX)', semeur: 'part2.ts:28', cheminHumain: 'imports/actions.ts:66 (même service que le NEP, dossier différent)', clique: null, etat: 'non_prouve',
      ...
      { objet: 'rebuildFslis (SOX)', semeur: 'part2.ts:29', cheminHumain: 'scoping/page.tsx:53', clique: null, etat: 'non_prouve' },
      { objet: 'matérialité proposée/validée (SOX)', semeur: 'part2.ts:30', cheminHumain: 'materiality/page.tsx:42,57', clique: null, etat: 'non_prouve' },
      { objet: 'RCM du cycle importé (importRcm)', semeur: 'part2.ts:31', cheminHumain: 'rcm/page.tsx:48-51 (vrai upload de fichier, uploadRcmAction dans rcm/actions.ts — Lot 7 H-5 tranche 1, remplace l’ancien bouton-démo qui relisait dataset/sox/rcm.csv depuis le disque du serveur)', clique: null, etat: 'non_prouve',
        note: 'R107 (docs/BACKLOG_REPORTE.md) LEVÉ le 2026-09-17 — mais SEULEMENT pour engNep (voir la ligne enrichir.ts:388 ci-dessous, désormais prouve) : CE dossier-ci est engSox, dont la RCM reste importée par bootstrapSox/runPart2 AVANT que clics ne s’exécute (demo-seed.ts l’appelle), donc controls.length>0 quand clics visite /rcm sur engSox — le bouton n’y est jamais offert. Pas le même défaut que R107 : R107 portait sur engNep, où l’énoncé était FAUX (vérifié par requête directe après db:reset && demo:seed) ; ici l’énoncé reste VRAI.' },
      { objet: 'lien de risque réel créé pour chaque contrôle importé — CTRL-02 (importRcm → control_risk)', semeur: 'sox.ts:110-124 (appelé depuis part2.ts:31 ; mandat contrôle interne, 2026-09-08, tranche 2)', cheminHumain: 'rcm/[cid]/page.tsx (lierRisqueAction, pour un lien SUPPLÉMENTAIRE — celui d’importRcm existe déjà à l’import)', clique: null, etat: 'non_prouve',
        note: 'domaine SOX entier hors du parcours cliqué (note ci-dessus) — même état que le reste de cette section, pas une régression neuve' },
      { objet: 'statut du walkthrough DI (setDiStatus)', semeur: 'part2.ts:140', cheminHumain: 'rcm/page.tsx:42', clique: 'scenario.ts:4107-4221 (station « la ladder de refus CTRL-01/02/03 », conclusion réussie après avoir levé les trois refus, C-REV-03)', etat: 'prouve',
        note: 'INVENTAIRE DE CLÔTURE §3 (2026-09-19) : la même station observe aussi les DEUX formes du refus CTRL-01 avant que ce geste ne réussisse — voir docs/CLOTURE.md.' },
      { objet: 'enregistrement vidéo du walkthrough attaché (attacherWalkthrough)', semeur: 'part2.ts:119 (mandat contrôle interne, 2026-09-08)', cheminHumain: 'rcm/[cid]/page.tsx (attacherWalkthroughAction)', clique: 'scenario.ts:4107-4221 (même station, C-REV-03 — dépôt réel PUIS ré-attachement après suppression VID-01)', etat: 'prouve' },
      { objet: 'tâche du walkthrough documentée (ajouterTacheControle)', semeur: 'part2.ts:120', cheminHumain: 'rcm/[cid]/page.tsx (ajouterTacheAction)', clique: 'scenario.ts:4107-4221 (même station, C-REV-03)', etat: 'prouve' },
      { objet: 'procédure de tâche documentée hors inquiry — CTRL-01 (documenterProcedureTache)', semeur: 'part2.ts:121', cheminHumain: 'rcm/[cid]/page.tsx (documenterProcedureAction)', clique: 'scenario.ts:4107-4221 (même station, C-REV-03 — c’est ce geste précis qui LÈVE le refus CTRL-01 observé juste avant)', etat: 'prouve' },
      { objet: 'facteur de design documenté, les quatre — CTRL-02 (documenterFacteurDesign)', semeur: 'part2.ts:131-138 (mandat contrôle interne, 2026-09-08, tranche 2)', cheminHumain: 'rcm/[cid]/page.tsx (documenterFacteurAction)', clique: 'scenario.ts:4107-4221 (même station, C-REV-03 — les QUATRE facteurs, un par un)', etat: 'prouve' },
      { objet: 'IUC déclarée (utilisée ou non) — CTRL-03 (declarerIuc)', semeur: 'part2.ts:139 (mandat contrôle interne, 2026-09-08, tranche 2)', cheminHumain: 'rcm/[cid]/page.tsx (declarerIucAction)', clique: 'scenario.ts:4107-4221 (même station, C-REV-03 — déclarée utilisee=oui, contrairement au semeur qui ne déclare que utilisee=false)', etat: 'prouve',
        note: 'documenterIucPreuve (preuves d’exactitude/exhaustivité) reste HORS du périmètre de ce registre (aucun fichier du semeur ne l’appelle, règle du fichier ligne 28) mais est désormais cliqué de façon DURABLE par la même station (les deux volets, C-REV-03) — remplace la vérification manuelle ponctuelle notée ici avant le 2026-09-19 (script jetable, supprimé).' },
      { objet: 'demande de listing formelle (insert request/request_item ad hoc)', semeur: 'part2.ts:42-50',
        cheminHumain: null, clique: null, etat: 'decor',
        note: 'le chemin cliquable (rcm/[cid]/page.tsx importInstancesAction) importe le CSV directement, sans créer de request formelle au préalable — un geste DIFFÉRENT de celui du semeur, pas un équivalent. DISTINCT de demanderPopulationControle (CTRL-05, population d’OCCURRENCES d’un contrôle as_needed, prouvé le 2026-09-19 par scenario.ts:4222-4241) : deux mécanismes de demande différents, l’un pour un LISTING client, l’autre pour une POPULATION d’occurrences — pas le même objet.' },
      { objet: 'demande envoyée + pièce reçue (approveSend, ingestEvidence — listing)', semeur: 'part2.ts:51,55-63', cheminHumain: 'requests/[rid]/page.tsx:33 ; rcm/[cid]/page.tsx (dépôt)', clique: null, etat: 'non_prouve' },
      { objet: 'occurrences importées (importInstances)', semeur: 'part2.ts:64', cheminHumain: 'rcm/[cid]/page.tsx:162,424', clique: null, etat: 'non_prouve' },
      { objet: 'pièce d’exécution de contrôle déposée (uploadControlEvidence)', semeur: 'part2.ts:73 (appelée depuis 155, 172)', cheminHumain: 'rcm/[cid]/page.tsx (dépôt sur demande d’échantillon)', clique: null, etat: 'non_prouve' },
      { objet: 'échantillon d’attributs tiré ET demande de preuves envoyée (drawAttributeSample + approveSend, DEUX instructions du MÊME drawAction)', semeur: 'part2.ts:150-154', cheminHumain: 'rcm/[cid]/page.tsx:171,426 (un seul bouton, un seul geste)', clique: null, etat: 'non_prouve',
        note: 'CORRIGÉ (constat D5, relecture hostile) : la version d’origine comptait ces deux instructions comme deux OBJETS distincts alors que c’est un seul geste humain (un clic) qui les produit toutes les deux — fusionnées ici, le compte total en est réduit d’une ligne. Lignes recomputées après CTRL-07 (tranche 3, §3.2) : le tirage de démonstration passe désormais par la dérogation écrite ADR-010 (part2.ts:143-153), un choix humain documenté, pas un défaut de pack tu — même geste, même objet.' },
      { objet: 'extraction du contrôle exécuté (extractAll/verifyExtraction, SOX)', semeur: 'part2.ts:156,158,173,175', cheminHumain: 'testing/page.tsx:67 (même service, dossier SOX)', clique: null, etat: 'non_prouve' },
      { objet: 'test d’efficacité exécuté (runAttributeTesting)', semeur: 'part2.ts:160,177', cheminHumain: 'rcm/[cid]/page.tsx:188,440', clique: null, etat: 'non_prouve' },
      { objet: 'extension à la population complète (extendToFullPopulation)', semeur: 'part2.ts:166', cheminHumain: null, clique: null, etat: 'decor',
        note: 'aucun appelant nulle part hors part2.ts et sa propre définition (sox.ts:929)' },
      { objet: 'déficience proposée (proposeDeficiency)', semeur: 'part2.ts:190', cheminHumain: 'rcm/[cid]/page.tsx:209,494', clique: null, etat: 'non_prouve' },
      { objet: 'déficience décidée (decideDeficiency)', semeur: 'part2.ts:197', cheminHumain: 'rcm/[cid]/page.tsx:221,557', clique: null, etat: 'non_prouve' },
      { objet: 'papier OE rédigé (draftOeWorkpaper)', semeur: 'part2.ts:199', cheminHumain: 'rcm/[cid]/page.tsx:233,503', clique: null, etat: 'non_prouve' },
    ],
  },
  {
    ...
    objets: [
      ...
      { objet: 'RCM du cycle (importRcm, revu depuis enrichir.ts)', semeur: 'enrichir.ts:388', cheminHumain: 'rcm/page.tsx:48-51 (même vrai upload que la ligne part2.ts, Lot 7 H-5 tranche 1)', clique: 'scenario.ts:3762 (station « R60 : les compteurs du dossier mènent quelque part », upload réel de dataset/sox/rcm.csv)', etat: 'prouve',
        note: 'R107 (docs/BACKLOG_REPORTE.md) LEVÉ le 2026-09-17 : l’énoncé d’origine (« jamais cliqué ») était FAUX pour engNep sur le monde LOCAL — demo-seed.ts n’appelle JAMAIS enrichirMondeDemo() (seul reconstruire.ts, production, le fait), donc engNep porte zéro contrôle à ce point du parcours canonique et le formulaire d’upload EST offert. Vérifié par requête SQL directe puis par npm run clics (F39, docs/CHASSE.md) avant d’écrire cette ligne — jamais supposé.' },
