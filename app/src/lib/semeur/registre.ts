...
export const SECTIONS: SectionSemeur[] = [
  ...
  },
  {
    ...
    objets: [
      ...
      { objet: 'demande PBC générée depuis la sélection (generatePbcFromSample)', semeur: 'part1.ts:100', cheminHumain: 'sampling/page.tsx:61-68 (pbcAction)', clique: 'scenario.ts:1086-1090 (bouton samp.generatePbcRequest cliqué, aucun refus détecté)', etat: 'prouve',
        note: 'LE CLIC EST PROUVÉ ; LE RÉSULTAT NE L’EST PAS — et c’est faux qu’il soit correct. Mesuré ce jour (2026-09-06, base fraîche, après le correctif R41) : `event_log` ne porte qu’UNE seule ligne `request_generated` sur tout le dossier, celle du SEMEUR (seq_no 1, 09:11:17) — aucune seconde ligne au moment où la station clique ce bouton. La station rend pourtant « demande engendrée » (aucun `?erreur=` sur l’URL finale). C’est très exactement R37 (docs/CHASSE.md §3, docs/BACKLOG_REPORTE.md) : ce registre ne juge que le CHEMIN, pas si l’action a réellement écrit — et ici, elle ne l’a pas fait.' },
