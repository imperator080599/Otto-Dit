...
export const SECTIONS: SectionSemeur[] = [
  ...
  },
  {
    ...
    objets: [
      ...
      { objet: 'RCM du cycle importé (importRcm)', semeur: 'part2.ts:31', cheminHumain: 'rcm/page.tsx:48-51 (vrai upload de fichier, uploadRcmAction dans rcm/actions.ts — Lot 7 H-5 tranche 1, remplace l’ancien bouton-démo qui relisait dataset/sox/rcm.csv depuis le disque du serveur)', clique: null, etat: 'non_prouve',
        note: 'R107 (docs/BACKLOG_REPORTE.md) : le geste est désormais RÉEL (vraie provenance import_file, kind=rcm) mais toujours non cliqué — le bouton ne s’affiche que si controls.length===0, et engSox comme engNep ont déjà leur RCM importée par le semeur avant que clics ne s’exécute.' },
  ...
  },
  {
    ...
    objets: [
      ...
      { objet: 'RCM du cycle (importRcm, revu depuis enrichir.ts)', semeur: 'enrichir.ts:388', cheminHumain: 'rcm/page.tsx:48-51 (même vrai upload que la ligne part2.ts, Lot 7 H-5 tranche 1)', clique: null, etat: 'non_prouve',
        note: 'R107 (docs/BACKLOG_REPORTE.md) : même route que la ligne part2.ts, même geste désormais réel, toujours non cliqué (engNep a déjà sa RCM importée par ce même semeur avant que clics ne s’exécute).' },
