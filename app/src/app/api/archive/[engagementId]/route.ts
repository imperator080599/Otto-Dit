...
// `exited_on` — un membre sorti pouvait encore télécharger l'archive scellée du dossier.

export async function GET(_req: NextRequest, ctx: { params: Promise<{ engagementId: string }> }) {
  const { engagementId } = await ctx.params;
  const user = await getSessionUser();
  if (!user) return new NextResponse('unauthorized', { status: 401 });
  if (!(await activeMembership(engagementId, user.id))) return new NextResponse('not found', { status: 404 });
  const a = await q01<{ storage_path: string; sha256: string; sealed_at: string }>(
    `select storage_path, sha256, sealed_at::text
