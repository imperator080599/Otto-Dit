...
const { restes: services, differes } = libellesDeService(lib, new Set(Object.keys(LIBELLES)));
...
if (process.argv.includes('--differes')) for (const d of differes) console.log(`différé · ${d}`);
console.log(`\n${restes.length} chaîne(s) d’écran hors catalogue · ${services.length} libellé(s) en dur dans un service`
  + ` · ${differes.length} libellé(s) différé(s) avec raison (--differes pour les lire) · ${refus} message(s) de refus`
