...
**Compte** : 89 objet(s)/geste(s) recensé(s) sur les cinq fichiers du semeur · **16 DÉCOR** (aucun chemin humain) · **25 non prouvé(s)** (chemin humain existant, jamais cliqué) · 48 prouvé(s).
...
| statut du walkthrough DI (setDiStatus) | `part2.ts:140` | `rcm/page.tsx:42` | `scenario.ts:4107-4221 (station « la ladder de refus CTRL-01/02/03 », conclusion réussie après avoir levé les trois refus, C-REV-03)` | prouvé |
| | | | | ↳ INVENTAIRE DE CLÔTURE §3 (2026-09-19) : la même station observe aussi les DEUX formes du refus CTRL-01 avant que ce geste ne réussisse — voir docs/CLOTURE.md. |
| enregistrement vidéo du walkthrough attaché (attacherWalkthrough) | `part2.ts:119 (mandat contrôle interne, 2026-09-08)` | `rcm/[cid]/page.tsx (attacherWalkthroughAction)` | `scenario.ts:4107-4221 (même station, C-REV-03 — dépôt réel PUIS ré-attachement après suppression VID-01)` | prouvé |
| tâche du walkthrough documentée (ajouterTacheControle) | `part2.ts:120` | `rcm/[cid]/page.tsx (ajouterTacheAction)` | `scenario.ts:4107-4221 (même station, C-REV-03)` | prouvé |
| procédure de tâche documentée hors inquiry — CTRL-01 (documenterProcedureTache) | `part2.ts:121` | `rcm/[cid]/page.tsx (documenterProcedureAction)` | `scenario.ts:4107-4221 (même station, C-REV-03 — c’est ce geste précis qui LÈVE le refus CTRL-01 observé juste avant)` | prouvé |
| facteur de design documenté, les quatre — CTRL-02 (documenterFacteurDesign) | `part2.ts:131-138 (mandat contrôle interne, 2026-09-08, tranche 2)` | `rcm/[cid]/page.tsx (documenterFacteurAction)` | `scenario.ts:4107-4221 (même station, C-REV-03 — les QUATRE facteurs, un par un)` | prouvé |
| IUC déclarée (utilisée ou non) — CTRL-03 (declarerIuc) | `part2.ts:139 (mandat contrôle interne, 2026-09-08, tranche 2)` | `rcm/[cid]/page.tsx (declarerIucAction)` | `scenario.ts:4107-4221 (même station, C-REV-03 — déclarée utilisee=oui, contrairement au semeur qui ne déclare que utilisee=false)` | prouvé |
| | | | | ↳ documenterIucPreuve (preuves d’exactitude/exhaustivité) reste HORS du périmètre de ce registre (aucun fichier du semeur ne l’appelle, règle du fichier ligne 28) mais est désormais cliqué de façon DURABLE par la même station (les deux volets, C-REV-03) — remplace la vérification manuelle ponctuelle notée ici avant le 2026-09-19 (script jetable, supprimé). |
| demande de listing formelle (insert request/request_item ad hoc) | `part2.ts:42-50` | — | — | **DÉCOR** |
| | | | | ↳ le chemin cliquable (rcm/[cid]/page.tsx importInstancesAction) importe le CSV directement, sans créer de request formelle au préalable — un geste DIFFÉRENT de celui du semeur, pas un équivalent. DISTINCT de demanderPopulationControle (CTRL-05, population d’OCCURRENCES d’un contrôle as_needed, prouvé le 2026-09-19 par scenario.ts:4222-4241) : deux mécanismes de demande différents, l’un pour un LISTING client, l’autre pour une POPULATION d’occurrences — pas le même objet. |
...
- **client_contact** (`seed.ts:229-236`) — recherche exhaustive : SEUL seed.ts:229 insère cette table dans tout app/src ; ailleurs elle n’est que lue ou jointe (reunions.ts). Citation de ligne corrigée après P2-03b (revue hostile V2-05) : le troisième contact fixture (Zoé, jeton toujours expiré, PORTAIL-02) a déplacé l’INSERT, la classification decor reste inchangée.
...
- **demande de listing formelle (insert request/request_item ad hoc)** (`part2.ts:42-50`) — le chemin cliquable (rcm/[cid]/page.tsx importInstancesAction) importe le CSV directement, sans créer de request formelle au préalable — un geste DIFFÉRENT de celui du semeur, pas un équivalent. DISTINCT de demanderPopulationControle (CTRL-05, population d’OCCURRENCES d’un contrôle as_needed, prouvé le 2026-09-19 par scenario.ts:4222-4241) : deux mécanismes de demande différents, l’un pour un LISTING client, l’autre pour une POPULATION d’occurrences — pas le même objet.
...
- **RCM du cycle importé (importRcm)** — chemin : `rcm/page.tsx:48-51 (vrai upload de fichier, uploadRcmAction dans rcm/actions.ts — Lot 7 H-5 tranche 1, remplace l’ancien bouton-démo qui relisait dataset/sox/rcm.csv depuis le disque du serveur)` — R107 (docs/BACKLOG_REPORTE.md) LEVÉ le 2026-09-17 — mais SEULEMENT pour engNep (voir la ligne enrichir.ts:388 ci-dessous, désormais prouve) : CE dossier-ci est engSox, dont la RCM reste importée par bootstrapSox/runPart2 AVANT que clics ne s’exécute (demo-seed.ts l’appelle), donc controls.length>0 quand clics visite /rcm sur engSox — le bouton n’y est jamais offert. Pas le même défaut que R107 : R107 portait sur engNep, où l’énoncé était FAUX (vérifié par requête directe après db:reset && demo:seed) ; ici l’énoncé reste VRAI.
