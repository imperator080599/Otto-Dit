...
- **#418 (hydratation)** : NON reproduit sur les sept parcours locaux du jour, mais OBSERVÉ une fois en ligne par le harnais d'acceptation (run 33643239966, tâche A-05, page du papier de travail REV-01, « Minified React error #418 ; args[]=HTML » (= divergence de STRUCTURE ; voir ADR-132)) — une seule occurrence sur deux passages déployés. Aucune cause prouvée ; W5 reste reporté avec cette observation en plus (docs/BACKLOG_REPORTE.md). Le job `url` de la CI reste ROUGE tant que cette exception survient : c'est voulu.
