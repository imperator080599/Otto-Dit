...
- **Suite**: 90 tests green (`cd app && npm test`), zero network calls. Prod build clean.
...
2. **Founder review item #2 — real-corpus extraction eval** (A11/A12): live rungs 3–5 on a
   permissioned real corpus + timed L2 verification benchmark. Cannot be run in this
   synthetic-only repo; gates any pilot.
3. Wire live inbound email transport (Q12) — the first deployment task.
4. Legal: secret professionnel / GDPR analysis + DPA (A13) before any real client data.
5. First fast-follows if the wedge holds: analytical review + auto variance questions,
