...
const S = {
  ...
  procOuverte:null, ctrTout:{}, filtres:{ statut:'', section:'', domaine:'', contact:'', echeance:'', q:'' },
  ...
  questEntite:{},
  /* Navigation : un seul groupe de rail déployé, et le filtre des sections. */
  railGroupe:null, railQ:'', railFiltre:'',
...
/* ═══ QUESTIONNAIRE RÉSIDUEL DE RISQUE ═════════════════════════════════════
   Il n'est PAS ici : c'est de la méthode, pas du code de démonstration. Les
   questions, la nature de risque inhérent qu'elles portent, la raison pour
   laquelle chacune existe encore et l'effet d'une réponse « oui » vivent dans
   methodology/questionnaire.json, versionnés, validés contre
   methodology/schema-questionnaire.json par le même valideur que le catalogue
   de procédures, et intégrés à la construction (_catalogue.gen.js) — d'où
   viennent NATURES_RI et QUESTIONNAIRE.

   Leur dérivation par portée se lit dans 17_facteurs.js, après le catalogue :
   déclarée ici, elle s'évaluerait avant lui.
   ═══════════════════════════════════════════════════════════════════════ */
...
function ancienneteRetard(r){
  ...
  /* ouvrePortail() et non « pas un week-end, sauf samedi ouvré » : cette
     seconde formulation comptait le DIMANCHE comme ouvré dès qu'on ouvrait le
     samedi. Une règle de jours ouvrés, une seule écriture. */
  while (d < S.aujourdhui){ d = addDays(d, 1); if (ouvrePortail(d)) n++; }
  return n;
}
